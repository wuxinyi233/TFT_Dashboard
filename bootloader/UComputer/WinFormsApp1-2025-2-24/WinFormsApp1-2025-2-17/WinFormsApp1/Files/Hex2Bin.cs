...
namespace WinFormsApp1.Files
{
    ...
    public static class MyHex2Bin
    {
        ...
        /// <param name="log">日志</param>
        /// <param name="inHexFilePath">hex文件路径</param>
        public static bool LoadHex(Encoding encoding, ref List<Section> sections, out string log, params string[] inHexFilePath)
        {
            log = "";
            bool fail = false;
            foreach (var item in inHexFilePath)
            {
                if (!string.IsNullOrWhiteSpace(item))//文件不为空
                {
                    //文件存在并且以.hex结尾
        ...
        /// <param name="ignoreByte">占位符 空白的地址填</param>
        /// <param name="sections">hex文件块信息</param>
        public static byte[] WriteBinBytes(byte ignoreByte, List<Section> sections)
        {
            uint startAddr = sections[0].startAddr;
            int size = (int)(sections[sections.Count - 1].endAddr - startAddr) + 1;
            if (size > 1024 * 1024 * 20)
            ...
            byte[] binByteArray = new byte[size];
            ArrayList.Repeat(ignoreByte, size).CopyTo(binByteArray);

            //将地址块信息写入到内存数组中
            foreach (var item in sections)
            {
                item.data.ToArray().CopyTo(binByteArray, item.startAddr - startAddr);
            }
